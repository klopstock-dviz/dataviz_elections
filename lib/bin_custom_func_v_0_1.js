...
function main_bin(dataset, params_chart) {
	...
	var output = [], final = []
	//case when the chart has sub category field
	if (params_chart.sub_category_field && params_chart.sub_category_field !== "") {
	...
	//case when the chart don't has sub category field
	else {

		var array_to_bin = dataset.map(o=> o[params_chart.bin_params.bin_field])
