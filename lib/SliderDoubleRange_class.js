class SliderDoubleRange {
	...
	createChart(params_chart, sharedParams) {
		...
		console.log('slider prepare_data_p1 time: ' + (new Date() - t1))

		var t1 = new Date()
		this.init_slider(params_chart, sharedParams)
		console.log('slider init_slider time: ' + (new Date() - t1))

		params_chart.chart_type = "slider"
		params_chart.chart_sub_type = "slider"
	...
	updateChart(params_chart, sharedParams) {
    	...
    	if (params_chart.rangeDataset.min !== undefined && params_chart.rangeDataset.max !== undefined) {
		...
		if (min === max && min !== 0) {
			min = min - (min/100);
			document.getElementById(params_chart.htmlNode + "_outerGrid").style.display = 'grid'; rangeCell.innerHTML = '';
		}
		else if (min+max !== 0) {
			document.getElementById(params_chart.htmlNode + "_outerGrid").style.display = 'grid'; rangeCell.innerHTML = '';
		}
		else if (min === max && min === 0) {
			min = -0.1; max = 0.1; rangeCell.innerHTML = 'no data'; document.getElementById(params_chart.htmlNode + "_outerGrid").style.display = 'none'
		}
	...
	async prepare_data_p1(params_chart, sharedParams) {
	    var d1 = new Date();
		...
		if (!params_chart.range_field_created && params_chart.data_params.join_params) {
			...
			Object.values(groups).forEach(r=> {
				...
				//build primary array iot create the selection value in the fact table
				//v1
				...
				//v2
				input_join[r[join_field_source]] = selection
				lookupTable.push({[join_field_source]: r[0][join_field_source], [range_field]: selection})
			})

			//create the selection value in the fact table
			join_v2(sharedParams.data_main, lookupTable, join_field_target, join_field_source, [range_field])
			console.log('slider selection params time: ' + (new Date() - t1))
	...
	async init_slider(params_chart, sharedParams) {
		...
		if (params_chart.tooltips) {	    		
			...
			params_chart.title_params.fontWeight ? title.style.fontWeight = params_chart.title_params.fontWeight : title.style.fontWeight = '500'
			title.innerHTML = params_chart.title_params.label
			params_chart.title_params.fontSize ? title.style.fontSize = params_chart.title_params.fontSize : title.style.fontSize = "1rem";
		...
		var range_Dataset = await params_chart.rangeDataset
    	var min = range_Dataset.min, max = range_Dataset.max;
    	
    	if (min === max && min !== 0) {min = min - (min/100)} 
    	else if (min === max && min === 0) {
    		min = -0.1; max = 0.1
    		rangeCell.innerHTML = 'no data';
    		document.getElementById(params_chart.htmlNode + "_outerGrid").style.display = 'none'
    	}
		...
		if (params_chart.tooltips) {
	    	if (params_chart.tooltips && !isNaN(params_chart.tooltips.decimals)) {var decimal = params_chart.tooltips.decimals}
	    	else {var decimal = 2}
		...
		slider.noUiSlider.on(params_chart.listen_method, function (values, handle1, handle1_2) {
			...
			params_chart.brush_keys_values = {[field]: [min+"_"+max]}; params_chart.brush_values = {start: min, end: max}; 
			params_chart.selected_values = {field: params_chart.data_params.range_field, operation: "between", valueMin: min, valueMax: max};
