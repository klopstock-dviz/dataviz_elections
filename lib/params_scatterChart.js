...
/*    mobx.extendObservable(this, {
    ...
    this.ctx = ''
    this.chart_sub_type = 'scatter'
    this.shape = {type: 'scatter', fill: false}
    ...
    this.selection_params = {interaction_mode: "filter", selection_level: "all", selection_field: "", highlight_mode: "all", brush:{active: true, mode: "brushEndEvent"}}
    ...
    //brush mode = brushEndEvent means that the colors of active points and the filtering process are activated at the end of the brush movement 
    //brushEvent & highlightEvent for continious refresh, brushEndEvent & highlightEndEvent for a refresh at the end of brush move/sizing
    ...
    this.hidden_legends = {}
    this.legends_field = ""
    this.limit_delay_animation = 4000 //used is state management script, beyond this limit of data length, the chart is replaced by a spinner while the calculations are done
    this.init_render = true
    this.delay_update = 1400 //delay in ms before updating the chart
    this.limit_decimation = 2500 //used is prepare_data_p1 script, beyond this limit of data length, the dataset is processed by the data_decimation script iot reduce its length & ensure smooth animation
    this.id_previous_singleSelect = ""
    this.id_previous_multiSelect = ""    
    this.funcLib = {}
    this.brush_mode = true
    this.radius_params = {min_radius: 1, max_radius: 5}
    this.style ={chart_width: 450, aspectRatio: 0.625, boxShadow: true, marginTop: 0}
  }
}
